...
class SaleController extends Controller
{
    ...
    {
        $saleItem = SaleItem::with('product')->where('sale_id', $id)->get();
        $sale = Sale::with('user')->where('id', $id)->first();

        return view('components.print', compact('saleItem', 'sale'));
    }
     ...
     public function report(Request $request)
{
    ...
    if ($request->start_date && $request->end_date){
        ...
        $sales = Sale::with('saleItems.product')
            ->orderBy('created_at', 'desc')
            ->paginate(7);
        return view('components.sales', compact('sales', 'startDate', 'endDate'));
    }
