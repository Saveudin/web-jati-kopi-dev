                            ...
                            <tr class="border-b">
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <div class="flex justify-center m-5">
                                        ...
                                        data-modal-target="updateProductModal{{ $item->id }}" data-modal-toggle="updateProductModal{{ $item->id }}" class="block text-white bg-black hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button">
                                        Update recipe
                                        </button>
                                    </div>
                                </td>
                                        ...
                                        <form action="{{ route('recipe.update', ['id' => $item->id]) }}" method="POST">
                                            ...
                                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                                <div>
                                                    ...
                                                    <select id="product" name="product_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                                                ...
                                                <div>
                                                    <label for="materials" class="block mb-2 text-sm font-medium text-gray-900">Ingredients</label>
                                                    <select name="raw_material_id" class="mb-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                                                        @foreach ($materials as $material)
                                                            <option value="{{ $material->id }}" {{ $material->id == $item->raw_material_id ? 'selected' : '' }}>{{ $material->name }}</option>
                                                        @endforeach
                                                    </select>
                                                    <input class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" type="number" step="0.01" name="quantity" value="{{ $item->quantity }}" required>
                                                    <select id="unit" name="unit" class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div id="ingredients">
                        <div class="ingredient-row">
                            <label for="materials" class="block mb-2 text-sm font-medium text-gray-900">Ingredients</label>
                            <select name="raw_materials[]" class="mb-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                                @foreach ($materials as $material)
                                    <option value="{{ $material->id }}">{{ $material->name }}</option>
                                @endforeach
                            </select>
                            <input class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" type="number" step="0.01" name="quantities[]" placeholder="Quantity" required>
                        <select id="unit" name="unit" class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                            <option value="" disabled selected>Select a unit</option>
                            <option value="gram">gram</option>
                            <option value="mililiter">mililiter</option>
                        </select>
                            <button type="button" class="remove-row mt-2 text-white inline-flex items-center bg-black hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Remove</button>
                        </div>
                    </div>
                    </div>
                    <button type="button" id="add-row" class="text-white inline-flex items-center bg-black hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">+ Add Ingredient</button>
                    <button type="submit" class="text-white inline-flex items-center bg-black hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Save recipe</button>
                </div>
            </form>
        </div>
    </div>
